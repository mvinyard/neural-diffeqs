...
class ResidualNeuralSDE(BaseSDE):
    DIFFEQ_TYPE = "SDE"
    ...
    def drift(self, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        y_hat = self.mu(y) * self._coef_drift
    ...
    def diffusion(self, y: torch.Tensor) -> torch.Tensor:
        ...
        y_hat = self.sigma(y).view(y.shape[0], y.shape[1], self._brownian_dim) * self._coef_diffusion
        y_ = y.view(y.shape[0], y.shape[1], self._brownian_dim)
